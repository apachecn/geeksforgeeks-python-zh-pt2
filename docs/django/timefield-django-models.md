...
> 原文:[https://www.geeksforgeeks.org/timefield-django-models/](https://www.geeksforgeeks.org/timefield-django-models/)

**时间字段**是一个存储时间的时间字段，在 Python 中由`datetime.time`实例表示。顾名思义，该字段用于存储用 python 创建的 datetime 对象。该字段的默认表单部件是`TextInput`。管理员使用两个独立的带有 JavaScript 快捷方式的`TextInput`小部件。
...
*   #### TimeField.auto_now
    ...
    每次保存对象时自动将字段设置为现在。对“上次修改”时间戳有用。请注意，始终使用当前时间；它不仅仅是一个可以覆盖的默认值。
...
*   #### TimeField.auto_now_add
    ...
    首次创建对象时，自动将字段设置为现在。对于创建时间戳很有用。请注意，始终使用当前时间；它不仅仅是一个可以覆盖的默认值。因此，即使您在创建对象时为该字段设置了值，它也将被忽略。如果您希望能够修改此字段，请设置以下内容，而不是 auto_now_add=True:

    *   对于**时间字段:默认=日期时间。时间。现在**–从`datetime.now()`
    *   对于**时间字段:默认=时区。现在**–从`django.utils.timezone.now()`开始
...
因此，当您在项目上运行迁移时，会创建一个 `geeks_field` **时间域**。是存储`datetime.time` python 对象的字段。

## 如何使用 TimeField？

TimeField 用于在数据库中存储 python datetime.time 实例。可以在数据库中使用相同的时间存储任何类型的时间。让我们尝试在上面创建的模型中存储一个日期。
...
![TimeField django models](img/79baad904a3f793b5e07a71b1ecb1926.png)

## 字段选项

字段选项是为每个字段提供的参数，用于对特定字段应用某些约束或赋予特定特征。例如，向时间字段添加参数`null = True`将使其能够在关系数据库中存储该表的空值。
以下是时间字段可以使用的字段选项和属性。
