...
> 原文:[https://www . geesforgeks . org/label _ 后缀-django-form-field-validation/](https://www.geeksforgeeks.org/label_suffix-django-form-field-validation/)
...
`label_suffix`用于在字段标签后追加一些文字(**默认为“:**)。`label_suffix`接受作为字段新标签后缀的字符串作为输入。字段的默认标签是通过将所有下划线转换为空格并大写第一个字母从字段名称生成的。让我们检查如何使用项目在字段中使用标签。
...
## 姜戈表单域验证`label_suffix`解释

使用示例说明标签后缀。考虑一个名为`geeksforgeeks`的项目，它有一个名为`geeks`的应用程序。
...
    geeks_field = forms.CharField(label_suffix = " - ")
```
...
![label_suffix-Django-Form-Field-Validation](img/1302645b11fadd34c8017fbe3402cb44.png)
因此，一个 `geeks_field` **字符域**被创建为带有**标签后缀“-**。
