...
> 原文:[https://www.geeksforgeeks.org/uuidfield-django-models/](https://www.geeksforgeeks.org/uuidfield-django-models/)

统一字段是存储**通用唯一标识符**的特殊字段。它使用 Python 的 [UUID](https://docs.python.org/3/library/uuid.html#uuid.UUID) 类。UUID，通用唯一标识符，是一个 python 库，帮助生成 128 位的随机对象作为 ID。它提供了唯一性，因为它根据时间、计算机硬件(MAC 等)生成 id。).通用唯一标识符是`primary_key`自动字段的一个很好的替代。数据库不会为你生成 UUID，建议使用`default`。
...
因此，当您在项目上运行 makemigrations 时，会创建一个 `geeks_field` **UUIDField** 。这是一个存储 python UUID 实例的字段。

## 如何使用 UUIDField？

UUID，通用唯一标识符，是一个 python 库，它帮助生成 128 位的随机对象作为 ID。要了解更多关于 UUID 的信息，请访问在 Python 中使用 UUID 生成随机 ID。让我们尝试将一个 uuid 对象保存到 UUIDField 中。
...
现在让我们在管理服务器中检查它。我们已经创建了一个极客模型
![UUIDField-django-models](img/79baad904a3f793b5e07a71b1ecb1926.png)的实例

## 字段选项

字段选项是为每个字段提供的参数，用于对特定字段应用某些约束或赋予特定特征。例如，向 UUIDField 添加参数`null = True`将使其能够在关系数据库中存储该表的空值。
以下是 UUIDField 可以使用的选项和属性。
