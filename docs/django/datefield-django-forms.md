...
> 原文:[https://www.geeksforgeeks.org/datefield-django-forms/](https://www.geeksforgeeks.org/datefield-django-forms/)

Django 表单中的日期字段是一个日期字段，用于从用户那里获取日期输入。该输入的默认小部件是[日期输入](https://docs.djangoproject.com/en/2.2/ref/forms/widgets/#django.forms.DateInput)。它规范化为:一个 Python **datetime.date** 对象。它验证给定值是 datetime.date、datetime.datetime 还是以特定日期格式格式化的字符串。

**日期字段有一个可选参数:**

**input_formats** :-用于尝试将字符串转换为有效 datetime.date 对象的格式列表。
...
因此，通过将“_”替换为“”，创建了一个 `geeks_field` **日期字段**。这是一个输入用户日期的字段。

## 如何使用 DateField？

日期字段用于输入数据库中的日期。可以输入提交日期、生日等。到目前为止，我们已经讨论了如何实现 DateField，但是如何在视图中使用它来执行逻辑部分。为了执行一些逻辑，我们需要将输入到字段中的值输入到 python 字符串实例中。
在视图中，
...
让我们尝试一下日期字段中的日期以外的其他内容。
![django-forms-datefield-validation](img/d6fa7730a55b791d41952e98f97b43f4.png)
...
![django-forms-datefield-input](img/757c9cbc9f0b8a0b58af66ac1424291f.png)

## 核心字段参数

核心字段参数是为每个字段提供的参数，用于对特定字段应用某种约束或赋予特定特征。例如，向日期字段添加参数`required = False`将使用户将其留空。每个字段类构造函数至少接受这些参数。有些字段类采用额外的、特定于字段的参数，但以下参数应始终被接受:
