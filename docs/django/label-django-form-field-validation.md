...
> 原文:[https://www . geesforgeks . org/label-django-form-field-validation/](https://www.geeksforgeeks.org/label-django-form-field-validation/)
...
`label`用于更改字段的显示名称。label 接受一个字符串作为输入，该字符串是字段的新名称。字段的默认标签是通过将所有下划线转换为空格并大写第一个字母从字段名称生成的。如果默认行为没有产生足够标签，请指定标签。让我们检查如何使用项目在字段中使用标签。
...
使用示例说明标签。考虑一个名为`geeksforgeeks`的项目，它有一个名为`geeks`的应用程序。
...
![label-–-Django-Form-Field-Validation](img/8e63c796627b4f4257562438d7258a10.png)

因此，一个名为“新极客领域”的 `geeks_field` **字符域**被创建。
