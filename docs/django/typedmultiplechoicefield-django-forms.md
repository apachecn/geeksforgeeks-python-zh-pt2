...
> 原文:[https://www . geeksforgeeks . org/typed multipleechicefield-django-forms/](https://www.geeksforgeeks.org/typedmultiplechoicefield-django-forms/)

Django Forms 中的 TypedMultipleChoiceField 是一个 Choice 字段，用于从一个字段中输入多对值，它还包括一个强制函数，用于将数据转换为特定的数据类型。该输入的默认部件是[选择多个](https://docs.djangoproject.com/en/2.2/ref/forms/widgets/#django.forms.SelectMultiple)。它规范化为一个 Python 字符串列表，您可以将其用于多种目的。

**type dmmultipleechicefield 有以下参数:**

*   **选项** :-用作该字段选项的二元组的可迭代表，或者返回这种可迭代表的可调用表。
*   **强制:**接受一个参数并返回强制值的函数。示例包括内置的 int、float、bool 和其他类型。默认为标识函数。
*   **empty_value :** 用于表示“空”的值默认为空字符串；这里没有其他常见的选择。
...
用一个例子说明类型多代码字段。考虑一个名为`geeksforgeeks`的项目，它有一个名为`geeks`的应用程序。
...
![django-forms-TypedMultipleChoiceField-11](img/629f5e4281fe5a3ad52be7e75e3cc208.png)

因此，通过将“_”替换为“”，创建了一个 `geeks_field` **类型的多重回声场**。它是从列表中输入选项的字段。

## 如何使用 TypedMultipleChoiceField？

TypedMultipleChoiceField 用于输入数据库中的选项。可以输入性别等。到目前为止，我们已经讨论了如何实现 TypedMultipleChoiceField，但是如何在视图中使用它来执行逻辑部分。为了执行一些逻辑，我们需要将输入到字段中的值输入到 python 字符串实例中。要获取工作类型为 TypedMultipleChoiceField 的 Github 代码，[点击此处](https://github.com/naveenkrnl/geeksforgeeks/tree/a657e1e497a2f62b54cdb16dffc3e7d5b1b11c0b)。
...
让我们现在尝试选择选择数据。
![django-forms-TypedMultipleChoiceField](img/5de45cf8e89a24a13f6b49c677e85db4.png)
...
![django-forms-typedchoicefield](img/47b682b8c64856b1bfc2e58b86e497fe.png)

## 核心字段参数

核心字段参数是为每个字段提供的参数，用于对特定字段应用某种约束或赋予特定特征。例如，添加一个参数`required = False`到 TypedMultipleChoiceField 将允许用户将其留空。每个字段类构造函数至少接受这些参数。有些字段类采用额外的、特定于字段的参数，但以下参数应始终被接受:
