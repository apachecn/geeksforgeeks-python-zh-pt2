...
> 原文:[https://www.geeksforgeeks.org/slugfield-django-forms/](https://www.geeksforgeeks.org/slugfield-django-forms/)

Django 表单中的 SlugField 是一个 slug 字段，用于输入特定网址或类似网址的 slug。该字段用于以表格形式表示模型 [SlugField](https://www.geeksforgeeks.org/slugfield-django-models/) 。该输入的默认部件是[文本输入](https://docs.djangoproject.com/en/2.2/ref/forms/widgets/#django.forms.TextInput)。它使用`validate_slug`或`validate_unicode_slug`来验证给定值只包含字母、数字、下划线和连字符。

**SlugField 有以下可选参数:**

*   **allow_unicode :-** 一个布尔值，指示字段接受 ASCII 字母之外的 unicode 字母。默认为假。
...
![django-forms-SlugField](img/3ea488448d74e5087ea3842203a5040a.png)

因此，通过将“_”替换为“”，创建了一个 `geeks_field` **SlugField** 。

## 如何使用 SlugField？

SlugField 用于通过 Django 表单输入数据库中的 slugs。可以输入网址段等。到目前为止，我们已经讨论了如何实现 SlugField，但是如何在视图中使用它来执行逻辑部分。为了执行一些逻辑，我们需要将输入到字段中的值输入到 python 字符串实例中。
在视图中，
...
![django-forms-slugfield](img/741a90f0fa957db0bf0efb7972635c30.png)
既然需要根据蛞蝓进行验证，那我们就在字段中输入“蛞蝓-django-models”

![1-django-forms-slugfield](img/0afd9376c7489c001bb3d69497a6fbca.png)
现在可以使用相应的请求字典来获取该数据。如果方法是 GET，数据将在**请求中可用。获取**和若帖，**请求。相应地开机自检**。在上面的例子中，我们有一个可以用于任何目的的 temp 值。

![django-forms-slugfield-3](img/b636c4970fc10cd76bd9c15a676d2f1f.png)

## 核心字段参数

核心字段参数是为每个字段提供的参数，用于对特定字段应用某种约束或赋予特定特征。例如，向 SlugField 添加一个参数`required = False`将使用户可以将其留空。每个字段类构造函数至少接受这些参数。有些字段类采用额外的、特定于字段的参数，但以下参数应始终被接受:
