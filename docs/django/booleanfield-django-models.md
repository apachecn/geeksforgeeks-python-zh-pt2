# BooleanField–Django 车型

> 原文:[https://www.geeksforgeeks.org/booleanfield-django-models/](https://www.geeksforgeeks.org/booleanfield-django-models/)

BooleanField 是一个**真/假**字段。它就像 C/C++中的 bool 字段。此字段的默认表单小部件是[复选框输入](https://docs.djangoproject.com/en/2.2/ref/forms/widgets/#django.forms.CheckboxInput)，如果为空=真，则选择[空框选择](https://docs.djangoproject.com/en/2.2/ref/forms/widgets/#django.forms.NullBooleanSelect)。当字段未定义时，BooleanField 的默认值为“无”。通过同时将默认属性设置为真/假，可以将默认值定义为真或假。
...
因此，当您在项目上运行迁移时，会创建一个 geeks_field **BooleanField** 。这是一个存储布尔值的字段-真/假。

## 如何使用 BooleanField？

BooleanField 用于检查特定条件，例如，检查用户的电子邮件是否经过验证。可以使用 BooleanField 来标记它是真还是假。我们也可以通过使用默认值=真将其设置为假。
...
![booleanfield django models](img/79baad904a3f793b5e07a71b1ecb1926.png)

### 字段选项

字段选项是为每个字段提供的参数，用于对特定字段应用某些约束或赋予特定特征。例如，将参数 null = True 添加到 BooleanField 将使其能够在关系数据库中存储该表的空值。
以下是 BooleanField 可以使用的字段选项和属性。
