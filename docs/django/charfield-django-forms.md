...
> 原文:[https://www.geeksforgeeks.org/charfield-django-forms/](https://www.geeksforgeeks.org/charfield-django-forms/)

Django Forms 中的 CharField 是一个字符串字段，用于从小到大的字符串。它用于接受用户的文本输入。该输入的默认部件是[文本输入](https://docs.djangoproject.com/en/2.2/ref/forms/widgets/#django.forms.TextInput)。如果提供了`max_length`和`min_length`，则使用`MaxLengthValidator`和`MinLengthValidator`。否则，所有输入都有效。
...
![django-forms-charfield](img/3ea488448d74e5087ea3842203a5040a.png)

因此，通过将“_”替换为“”，创建了一个 `geeks_field` **字符域**。它是一个输入小到大字符串的字段。

## 如何使用 CharField？

CharField 用于输入数据库中的小字符串。可以输入名字、姓氏、地址详情等。到目前为止，我们已经讨论了如何实现 CharField，但是如何在视图中使用它来执行逻辑部分。为了执行一些逻辑，我们需要将输入到字段中的值输入到 python 字符串实例中。
在视图中，
...
![django-charfield-forms](img/6ef2e230692307a6f6dfc7cb16638c1d.png)

现在可以使用相应的请求字典来获取这些数据。如果方法是 GET，数据将在**请求中可用。获取**和如果发布，**请求。相应地开机自检**。在上面的例子中，我们有一个可以用于任何目的的 temp 值。

![django-charfield-forms-output](img/10c2d89857a8454366de37da9b4b6cb9.png)

## 核心字段参数

核心字段参数是为每个字段提供的参数，用于对特定字段应用某种约束或赋予特定特征。例如，向字符域添加一个参数`required = False`将使用户可以将其留空。每个字段类构造函数至少接受这些参数。有些字段类采用额外的、特定于字段的参数，但以下参数应始终被接受:
