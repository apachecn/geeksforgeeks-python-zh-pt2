...
> 原文:[https://www.geeksforgeeks.org/textfield-django-models/](https://www.geeksforgeeks.org/textfield-django-models/)

文本字段是用于大尺寸文本的大文本字段。文本字段通常用于存储段落和所有其他文本数据。该字段的默认表单部件是[文本区域](https://docs.djangoproject.com/en/2.2/ref/forms/widgets/#textarea)。
...
使用示例说明文本字段。考虑一个名为`geeksforgeeks`的项目，它有一个名为`geeks`的应用程序。
...
因此，当您在项目上运行迁移时，会创建一个 `geeks_field` **文本字段**。它是一个存储小到大字符串的字段。

## 如何使用 TextField？

文本字段用于在数据库中存储大文本。可以存储段落、数据等。应该使用像`First_name`、`Last_name`、[字符域](https://www.geeksforgeeks.org/charfield-django-models/)这样的小文本来存储。让我们创建一个我们创建的文本字段的实例，并检查它是否工作。
...
![TextField django models](img/79baad904a3f793b5e07a71b1ecb1926.png)

## 字段选项

字段选项是为每个字段提供的参数，用于对特定字段应用某些约束或赋予特定特征。例如，向文本字段添加参数`null = True`将使其能够在关系数据库中存储该表的空值。
以下是文本字段可以使用的字段选项和属性。
