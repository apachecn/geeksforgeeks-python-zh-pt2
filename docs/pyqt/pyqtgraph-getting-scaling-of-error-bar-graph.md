# PyQtGraph–获取误差条形图的缩放比例

> 原文:[https://www . geesforgeks . org/pyqtgraph-get-scaling-of-error-bar-graph/](https://www.geeksforgeeks.org/pyqtgraph-getting-scaling-of-error-bar-graph/)

在本文中，我们将看到如何在 PyQtGraph 模块中获得误差条形图的比例。PyQtGraph 是 Python 的图形和用户界面库，提供设计和科学应用程序通常需要的功能。它的主要目标是为显示数据(图表、视频等)提供快速的交互式图形。).误差线是数据可变性的图形表示，用在图表上表示报告测量中的误差或不确定性。它们给出了测量精度的大致概念，或者相反，真实值可能与报告值有多远。缩放意味着增加或减少误差条形图数据的大小，通过缩放，我们以某种因子增加或减少误差条形图的整个数据。可以借助 setScale 方法进行设置。
...
![](img/bb704bd5ea7d8522aa35af39c2b836ce.png)