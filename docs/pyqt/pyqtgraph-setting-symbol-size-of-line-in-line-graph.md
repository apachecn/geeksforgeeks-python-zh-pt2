# PyQtGraph–设置线图中线条的符号大小

> 原文:[https://www . geesforgeks . org/pyqtgraph-setting-symbol-size-in-line-graph/](https://www.geeksforgeeks.org/pyqtgraph-setting-symbol-size-of-line-in-line-graph/)

在本文中，我们将看到如何在 PyQtGraph 模块的折线图中设置线的符号大小。PyQtGraph 是 Python 的图形和用户界面库，提供设计和科学应用程序通常需要的功能。它的主要目标是为显示数据(图表、视频等)提供快速的交互式图形。)折线图、线图、线图或曲线图是一种将信息显示为由直线段连接的一系列数据点(称为“标记”)的图表。它是许多领域中常见的一种基本图表类型。线图是借助 PyQtGraph 中的绘图类创建的。符号是连接线条数据的点的图形。主要是 x，o。
...
> 为此，我们对行对象
> **使用 setSymbolSize 方法语法:**line . setSymbolSize(n)
> **参数:**它以整数作为参数
> **返回:**它返回 None
...
        line1.setSymbolPen(QColor(220, 30, 30))
...
![](img/3f97a1594e3e95d612831502f6b478ce.png)