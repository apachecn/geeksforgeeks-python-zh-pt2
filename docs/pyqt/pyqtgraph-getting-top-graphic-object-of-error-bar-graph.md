...
> 原文:[https://www . geeksforgeeks . org/pyqtgraph-get-top-graph-错误对象条形图/](https://www.geeksforgeeks.org/pyqtgraph-getting-top-graphic-object-of-error-bar-graph/)

在本文中，我们将看到如何在 PyQtGraph 模块中获得错误条形图的顶部图形对象。PyQtGraph 是 Python 的图形和用户界面库，提供设计和科学应用程序通常需要的功能。它的主要目标是为显示数据(图表、视频等)提供快速的交互式图形。).误差线是数据可变性的图形表示，用在图表上表示报告测量中的误差或不确定性。它们给出了测量精度的大致概念，或者相反，真实值可能与报告值有多远。顶部图形对象是位于错误栏项目的堆栈小部件顶部的小部件对象。
...
> 为此，我们对错误栏项目对象
> **使用了 toGraphicsObject 方法语法:**error . toGraphicsObject()
> **参数:**它不需要参数
> **Return :** 它返回 QGraphic 对象
...
        # getting top graphic object of error bar item
        value = error.toGraphicsObject()
...
![](img/99a0840e3488e99f834313b4ae834aee.png)