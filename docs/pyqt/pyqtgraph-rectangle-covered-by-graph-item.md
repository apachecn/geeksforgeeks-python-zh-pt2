...
> 原文:[https://www . geeksforgeeks . org/pyqtgraph-矩形-被图形覆盖-item/](https://www.geeksforgeeks.org/pyqtgraph-rectangle-covered-by-graph-item/)

在本文中，我们将看到如何在 PyQTGaph 中获得图形项覆盖的矩形。PyQtGraph 是 Python 的图形和用户界面库，提供设计和科学应用程序通常需要的功能。它的主要目标是为显示数据(图表、视频等)提供快速的交互式图形。).图是由节点和边组成的非线性数据结构。节点有时也称为顶点，边是连接图中任意两个节点的直线或圆弧。该图由一组有限的顶点(或节点)和一组连接一对节点的边组成。矩形坐标将覆盖图形项目的所有节点。
...
> 为此，我们对图形项对象
> **使用 boundingRectmethod 语法:** imv.boundingRect()
> **参数:**不需要参数
> **返回:**它返回 QRect 对象
...
![](img/2ed4109a6c4b9a766ed8b7396b311cd8.png)