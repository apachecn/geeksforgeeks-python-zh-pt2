# PyQtGraph–在给定位置获取图像视图的子视图

> 原文:[https://www . geeksforgeeks . org/pyqtgraph-在给定位置获得图像视图的孩子/](https://www.geeksforgeeks.org/pyqtgraph-getting-child-of-image-view-at-given-position/)

在本文中，我们将看到如何在 PyQTGaph 中的给定位置获得图像视图对象的子对象。PyQtGraph 是 Python 的图形和用户界面库，提供设计和科学应用程序通常需要的功能。它的主要目标是为显示数据(图表、视频等)提供快速的交互式图形。).用于显示和分析图像数据的小部件。实现许多功能，如显示 2D 和三维图像数据。对于 3D 数据，显示 z 轴滑块，允许用户选择显示哪个帧。显示图像数据的直方图，可移动区域定义暗/亮级别，可编辑渐变提供颜色查找表。图像视图是一组小部件，像它包含一个视图对象，按钮和滚动条。
我们可以借助下面给出的命令
创建一个图像视图
...
        # getting child of image view at 10, 10
        value = imv.childAt(10, 10)
...
![](img/3b55c60ab622a91dc81896aca6b817d8.png)