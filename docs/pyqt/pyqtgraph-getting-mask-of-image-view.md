...
> 原文:[https://www . geeksforgeeks . org/pyqtgraph-get-mask-of-image-view/](https://www.geeksforgeeks.org/pyqtgraph-getting-mask-of-image-view/)

在本文中，我们将看到如何在 PyQTGraph 中获得图像视图对象的蒙版。PyQtGraph 是 Python 的图形和用户界面库，提供设计和科学应用程序通常需要的功能。它的主要目标是为显示数据(图表、视频等)提供快速的交互式图形。).用于显示和分析图像数据的小部件。实现许多功能，如显示 2D 和三维图像数据。对于 3D 数据，显示 z 轴滑块，允许用户选择显示哪个帧。显示图像数据的直方图，可移动区域定义暗/亮级别，可编辑渐变提供颜色查找表。蒙版基本上隐藏了图像视图的选定部分，图像视图将仍然在那里，但用户将无法看到它，即它上面将有一个蒙版。只有选中的区域会可见，其他区域会有一个蒙版，可以借助设置蒙版的方法进行设置。
...
        # getting mask of image view
        value = imv.mask()

        # setting text to the label
        label.setText("Mask : " + str(value))
...
![](img/37982876cbc50f3d4c08f3cb040538df.png)