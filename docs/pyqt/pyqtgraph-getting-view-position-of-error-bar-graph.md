...
> 原文:[https://www . geeksforgeeks . org/pyqtgraph-get-view-position-of-error-bar-graph/](https://www.geeksforgeeks.org/pyqtgraph-getting-view-position-of-error-bar-graph/)

在本文中，我们将看到如何在 PyQtGraph 模块中获得错误条形图的视图位置。PyQtGraph 是 Python 的图形和用户界面库，提供设计和科学应用程序通常需要的功能。它的主要目标是为显示数据(图表、视频等)提供快速的交互式图形。).误差线是数据可变性的图形表示，用在图表上表示报告测量中的误差或不确定性。它们给出了测量精度的大致概念，或者相反，真实值可能与报告值有多远。误差线项目的位置基本上是数据从原点开始的位置，默认为 0，尽管它可以随时改变。视图位置是用户看到错误条形图的位置。
...
        # getting view position of the error bar item
        value = error.viewPos()

        # setting text to the label
        label.setText("View Position : " + str(value))
...
![](img/7fe482ac44a91f052c0391aa5655f3e0.png)