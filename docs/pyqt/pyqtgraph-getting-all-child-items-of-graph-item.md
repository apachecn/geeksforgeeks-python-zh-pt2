# PyQtGraph–获取图形项目的所有子项目

> 原文:[https://www . geeksforgeeks . org/pyqtgraph-get-所有子项-of-graph-item/](https://www.geeksforgeeks.org/pyqtgraph-getting-all-child-items-of-graph-item/)

在本文中，我们将看到如何在 PyQTGaph 中获取图形项目的所有子项目。PyQtGraph 是 Python 的图形和用户界面库，提供设计和科学应用程序通常需要的功能。它的主要目标是为显示数据(图表、视频等)提供快速的交互式图形。).图是由节点和边组成的非线性数据结构。节点有时也称为顶点，边是连接图中任意两个节点的直线或圆弧。图由一组有限的顶点(或节点)和一组连接一对节点的边组成。所有子项目都是从该图形项目开始的整个项目树的列表。
...
> 为此，我们对图形项对象
> **使用 allChildItemsmethod 语法:** imv.allChildItems()
> **参数:**不需要参数
> **返回:**返回列表
...
![](img/b19d628404f6540360ef4ec791aaecb5.png)