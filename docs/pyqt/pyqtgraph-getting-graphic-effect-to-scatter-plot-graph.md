...
> 原文:[https://www . geeksforgeeks . org/pyqtgraph-get-graph-effect-to-散点图-graph/](https://www.geeksforgeeks.org/pyqtgraph-getting-graphic-effect-to-scatter-plot-graph/)

在本文中，我们将看到如何在 PyQtGraph 模块中获得散点图的图形效果。PyQtGraph 是 Python 的图形和用户界面库，提供设计和科学应用程序通常需要的功能。它的主要目标是为显示数据(图表、视频等)提供快速的交互式图形。).散点图(又称散点图，散点图)使用点来表示两个不同数值变量的值。这是一种使用笛卡尔坐标来显示一组数据的两个变量的值的绘图或数学图表。水平轴和垂直轴上每个点的位置表示单个数据点的值。图形效果可以是阴影、模糊、颜色甚至是定制效果，它用于使散点图的图形点有吸引力。可以借助 setGraphicsEffect 方法进行设置。
...
> 为此，我们对散点图对象
> **使用 graphicsEffect 方法语法:**散点图效果()
> **参数:**它不需要参数
> **返回:**它返回 qgraphicseffect 对象
...
![](img/2bd1c084d32c995fd98ab04779b4a464.png)