# PyQtGraph–检查误差条形图是否已启用

> 原文:[https://www . geesforgeks . org/pyqtgraph-checking-if-error-bar-graph-is-enabled/](https://www.geeksforgeeks.org/pyqtgraph-checking-if-error-bar-graph-is-enabled/)

在本文中，我们将了解如何在 PyQtGraph 模块中检查是否启用了错误条形图。PyQtGraph 是 Python 的图形和用户界面库，提供设计和科学应用程序通常需要的功能。它的主要目标是为显示数据(图表、视频等)提供快速的交互式图形。).误差线是数据可变性的图形表示，用在图表上表示报告测量中的误差或不确定性。它们给出了测量精度的大致概念，或者相反，真实值可能与报告值有多远。当错误栏项目处于活动状态(即正在工作)时，它会在停止或被删除时被启用，然后被禁用。
...
![](img/9babf033fd3128be1dd6b2ba8c649e3b.png)