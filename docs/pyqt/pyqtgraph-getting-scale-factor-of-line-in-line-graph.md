# PyQtGraph–获取线图中直线的比例因子

> 原文:[https://www . geesforgeks . org/pyqtgraph-get-scale-factor-in-line-graph/](https://www.geeksforgeeks.org/pyqtgraph-getting-scale-factor-of-line-in-line-graph/)

在本文中，我们将看到如何在 PyQtGraph 模块中获得折线图的比例因子。PyQtGraph 是 Python 的图形和用户界面库，提供设计和科学应用程序通常需要的功能。它的主要目标是为显示数据(图表、视频等)提供快速的交互式图形。)折线图或线图或折线图或曲线图是一种图表类型，它将信息显示为一系列由直线段连接的数据点，称为“标记”。它是许多领域中常见的一种基本图表类型。线图是借助 PyQtGraph 中的绘图类创建的。比例因子是用作线数据乘以的因子的因子。可以借助 setScale 方法进行设置。
...
![](img/5c6eb80af45571183615873d62e2f44a.png)