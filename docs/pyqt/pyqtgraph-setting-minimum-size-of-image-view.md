...
> 原文:[https://www . geeksforgeeks . org/pyqtgraph-setting-最小尺寸图像-view/](https://www.geeksforgeeks.org/pyqtgraph-setting-minimum-size-of-image-view/)

在本文中，我们将看到如何在 PyQTGaph 中设置图像视图的最小大小。PyQtGraph 是 Python 的图形和用户界面库，提供设计和科学应用程序通常需要的功能。它的主要目标是为显示数据(图表、视频等)提供快速的交互式图形。).用于显示和分析图像数据的小部件。实现许多功能，如显示 2D 和三维图像数据。对于 3D 数据，显示 z 轴滑块，允许用户选择显示哪个帧。显示图像数据的直方图，可移动区域定义暗/亮级别，可编辑渐变提供颜色查找表。最小尺寸是图像视图尺寸不能再减小的尺寸。设置最小尺寸比单独设置宽度和高度要好。
...
为此，我们对图像视图对象使用 **setMinimumSize()** 方法

> **语法:** imv.setMinimumSize(w，h)
> **参数:**取两个整数作为参数
> **返回:**不返回
...
        # setting minimum Size of image view
        imv.setMinimumSize(300, 300)
...
![](img/f6162e52bac1124a16dce8cb7cb98e2a.png)