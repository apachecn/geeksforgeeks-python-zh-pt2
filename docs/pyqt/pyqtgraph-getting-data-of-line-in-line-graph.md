# PyQtGraph–获取折线图中的直线数据

> 原文:[https://www . geesforgeks . org/pyqtgraph-获取线上线下数据图/](https://www.geeksforgeeks.org/pyqtgraph-getting-data-of-line-in-line-graph/)

在本文中，我们将看到如何在 PyQtGraph 模块中获取折线图的数据。PyQtGraph 是 Python 的图形和用户界面库，提供设计和科学应用程序通常需要的功能。它的主要目标是为显示数据(图表、视频等)提供快速的交互式图形。)折线图、线图、线图或曲线图是一种将信息显示为由直线段连接的一系列数据点(称为“标记”)的图表。它是许多领域中常见的一种基本图表类型。线图是借助 PyQtGraph 中的绘图类创建的。数据基本上是绘制线条的 x 轴和 y 轴值。
...
![](img/b521bfebd229775e0af85415f22f2220.png)