# PyQtGraph–获取图像视图的最大宽度/高度

> 原文:[https://www . geeksforgeeks . org/pyqtgraph-get-最大宽度-图像高度-view/](https://www.geeksforgeeks.org/pyqtgraph-getting-maximum-width-height-of-image-view/)

在本文中，我们将看到如何在 PyQTGraph 中获得图像视图的最大宽度和高度。PyQtGraph 是 Python 的图形和用户界面库，提供设计和科学应用程序通常需要的功能。它的主要目标是为显示数据(图表、视频等)提供快速的交互式图形。).用于显示和分析图像数据的小部件。实现许多功能，如显示 2D 和三维图像数据。对于 3D 数据，显示 z 轴滑块，允许用户选择显示哪个帧。显示图像数据的直方图，可移动区域定义暗/亮级别，可编辑渐变提供颜色查找表。最大高度和宽度是高度和宽度，在此之后图像视图高度或宽度不能再增加。虽然可以通过**设置最大值()**和**设置最大值()**方法进行设置，但默认情况下是一个很高的值。
...
为此，我们对图像视图对象使用**maximumwhat()**和**maximumidth()**方法

> **语法:**imv . maximumidth()和 imv . maximumidth()
> **参数:**不需要参数
> **返回:**返回整数
...
        # getting maximum width and height od image view
...
![](img/e4bb59780e7f9decd5ef2546c8a0eca2.png)