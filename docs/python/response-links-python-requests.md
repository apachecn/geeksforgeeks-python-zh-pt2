...
> 原文:[https://www . geesforgeks . org/response-link-python-requests/](https://www.geeksforgeeks.org/response-links-python-requests/)

**response.links** 返回标题链接。要了解更多关于 Http 报头的信息，请访问–[Http 报头](https://www.geeksforgeeks.org/http-headers/)。Python 请求通常用于从特定资源 URI 获取内容。每当我们通过 Python 向指定的 URI 发出请求时，它都会返回一个响应对象。现在，这个响应对象将用于访问某些特性，如内容、标题等。本文围绕如何检查响应对象中的**响应链接**展开。

#### 如何使用 Python 请求使用 response.links？

为了说明 response.links 的用法，我们来看看 api.github.com。要运行这个脚本，您需要在您的电脑上安装 Python 和请求。
...
![response.links-Python-requests](img/df9e1a43d29f70d4ab3356498ab2586f.png)

检查 **{}** 在输出的开始，它显示了标题链接 JSON。
