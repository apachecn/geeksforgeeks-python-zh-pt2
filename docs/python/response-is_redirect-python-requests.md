# response . is _ redirect–Python 请求

> 原文:[https://www . geesforgeks . org/response-is _ redirect-python-requests/](https://www.geeksforgeeks.org/response-is_redirect-python-requests/)

Python 请求通常用于从特定资源 URI 获取内容。每当我们通过 Python 向指定的 URI 发出请求时，它都会返回一个响应对象。现在，这个响应对象将用于访问某些特性，如内容、标题等。本文围绕如何检查响应对象中的**响应。 **response.is_redirect** 如果响应被重定向，则返回 **True** ，否则返回 **False** 。**

#### 如何使用 Python 请求使用 response.is_redirect？

为了说明 response.is_redirect 的用法，让我们 ping geeksforgeeks.org。要运行这个脚本，您需要在您的电脑上安装 Python 和请求。
...
![response.is_redirect-Python-requests](img/253e8c781dadc6f294a75ac3260270d7.png)

检查**是否为假**在输出开始时，显示 is_redirect 为假。
